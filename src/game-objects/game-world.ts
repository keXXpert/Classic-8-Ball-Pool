...
export class GameWorld {
    ...
    private handleInput(): void {
        if (AI.finishedSession && Mouse.isDown(inputConfig.mouseShootButton)) {
            if (this._stick.shot) {
    ...
    private resolveBallsCollision(first: Ball, second: Ball): boolean {
        ...
        // find velocity normal angle
        const alpha1 = Math.atan2(first.velocity.y, first.velocity.x) + Math.PI / 2
        const alpha2 = Math.atan2(second.velocity.y, second.velocity.x) + Math.PI / 2

        // calculate additional velocity due to Z rotation
        const velocity1 = first.rotation.z * ballConfig.diameter / 2 * physicsConfig.rotationToVelocity
        const velocity2 = second.rotation.z * ballConfig.diameter / 2 * physicsConfig.rotationToVelocity
        const alphaVel1 = new Vector2(velocity1 * Math.cos(alpha1), velocity1 * Math.sin(alpha1))
        const alphaVel2 = new Vector2(velocity2 * Math.cos(alpha2), velocity2 * Math.sin(alpha2))

        // calculate additional velocity due to positive XY rotation
        // const sameDirection1 = Math.sign(first.velocity.y) === Math.sign(first.rotation.x) && Math.sign(first.velocity.x) === Math.sign(first.rotation.y)
        // const sameDirection2 = Math.sign(second.velocity.y) === Math.sign(second.rotation.x) && Math.sign(second.velocity.x) === Math.sign(second.rotation.y)
        const xVelocity1 = first.rotation.y * ballConfig.diameter / 2 * physicsConfig.rotationToVelocity
        const yVelocity1 = first.rotation.x * ballConfig.diameter / 2 * physicsConfig.rotationToVelocity
        const xVelocity2 = second.rotation.y * ballConfig.diameter / 2 * physicsConfig.rotationToVelocity
        const yVelocity2 = second.rotation.x * ballConfig.diameter / 2 * physicsConfig.rotationToVelocity
        const rotVelocity1 = new Vector2(xVelocity1, yVelocity1)
        const rotVelocity2 = new Vector2(xVelocity2, yVelocity2)

        // Update velocities (summ velocities from hit & rotation)
        first.velocity = v1nTag.add(v1tTag);
        second.velocity = v2nTag.add(v2tTag);

        // add rotation velocities due Z rotation 
        if (alphaVel1.length) first.velocity = first.velocity.add(alphaVel1)
        if (alphaVel2.length) second.velocity = second.velocity.add(alphaVel2)

        // add rotation velocities due XY rotation (counter-rotation is not taken into account)
        // if (sameDirection1)
        first.velocity = first.velocity.add(rotVelocity1)
        //if (sameDirection2) 
        second.velocity = second.velocity.add(rotVelocity2)
    ...
    private drawHitMenu(): void {
        const ballSprite = Assets.getSprite(sprites.paths.hitMarker)

        Canvas2D.drawImage(ballSprite, GameConfig.hitMarketPosition, 0)
    }
    ...
    private createNumberedBalls(): void {
        this._8Ball = new Ball(Vector2.copy(GameConfig.eightBallPosition), Color.black, BallTypeEnum.Eight, 8);
        this._cueBall = new Ball(Vector2.copy(GameConfig.cueBallPosition), Color.white, BallTypeEnum.Cue, 0);

        // solid balls from 1 to 7
        const solidBalls: Ball[] = GameConfig.redBallsPositions
            .map((position: Vector2, i: number) => new Ball(Vector2.copy(position), Color.red, BallTypeEnum.Solid, i + 1));

        // stripe balls form 9 to 15
        const stripeBalls: Ball[] = GameConfig.yellowBallsPositions
            .map((position: Vector2, i: number) => new Ball(Vector2.copy(position), Color.yellow, BallTypeEnum.Stripe, i + 9));
    ...
    public initMatch(): void {
        ...
        this.createNumberedBalls()  // extracted to a separate function in case there would be two types of balls
    ...
    public isValidPosToPlaceCueBall(position: Vector2): boolean {
        const noOverlap: boolean = this._balls.every((ball: Ball) => {
    ...
    public shootCueBall(power: number, rotation: number): void {
        if (power > 0) {
            ...
            this._stick.shoot();
            this._cueBall.shoot(power, rotation, this._stick.hitPosition);
            this._stick.movable = false;
            this._stick.shot = false
